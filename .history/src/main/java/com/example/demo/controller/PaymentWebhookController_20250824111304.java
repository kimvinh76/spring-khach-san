...
public class PaymentWebhookController {
    ...
    public ResponseEntity<String> webhook(@RequestHeader(value = "X-Payment-Secret", required = false) String secret,
        ...
        try {
            ...
            // Provider-specific verification (if signature header present)
            if (signature != null && !signature.isEmpty() && !verifySignatureIfPresent(signature, rawBody)) {
            ...
            // Delegate processing (idempotency, payment record, invoice, booking update)
            processPaymentPayload(payload, booking);

            return ResponseEntity.ok("ok");
        } catch (java.io.IOException e) {
            LOGGER.error("webhook processing failed (io)", e);
    ...
    private String detectProviderFromRaw(String rawBody) {
        try {
            com.fasterxml.jackson.databind.ObjectMapper treeMapper = new com.fasterxml.jackson.databind.ObjectMapper();
            String provider = treeMapper.readTree(rawBody).path("provider").asText(null);
    ...
    private boolean verifySignatureIfPresent(String signature, String rawBody) {
        String provider = detectProviderFromRaw(rawBody);
        String secretToUse = webhookSecret;
        if (provider != null && paymentProperties != null) {
            if (provider.equalsIgnoreCase("stripe") && paymentProperties.getStripe() != null) {
                secretToUse = paymentProperties.getStripe().getWebhookSecret();
            } else if (provider.equalsIgnoreCase("momo") && paymentProperties.getMomo() != null) {
                secretToUse = paymentProperties.getMomo().getSecretKey();
            }
        }
        if (provider != null && provider.equalsIgnoreCase("stripe") && secretToUse != null) {
            return verifyStripeSignature(signature, rawBody, secretToUse);
        }
        return verifyHmacSignatureWithSecret(rawBody, signature, secretToUse);
    }

    /**
     * Verify Stripe-style signature header: 't=timestamp,v1=signature[,v1=...]'
     */
    private boolean verifyStripeSignature(String header, String payload, String secret) {
        if (header == null || header.isEmpty() || secret == null) return false;
        long toleranceSec = 300L; // 5 minutes
        try {
            ...
            for (String p : parts) {
                ...
                if (kv[0].trim().equals("t")) tPart = kv[1].trim();
                if (kv[0].trim().equals("v1")) v1.add(kv[1].trim());
            }
            if (tPart == null) return false;
            long ts = Long.parseLong(tPart);
            long now = java.time.Instant.now().getEpochSecond();
            if (Math.abs(now - ts) > toleranceSec) return false;

            String signedPayload = tPart + "." + payload;
            String expected = computeHmacSha256Hex(signedPayload, secret);
            for (String s : v1) {
                if (expected.equalsIgnoreCase(s)) return true;
            }
            return false;
        } catch (NumberFormatException | java.security.GeneralSecurityException ex) {
            LOGGER.warn("stripe signature verification error", ex);
            return false;
        }
    }
    ...
    mac.init(new javax.crypto.spec.SecretKeySpec(key.getBytes(java.nio.charset.StandardCharsets.UTF_8), HMAC_ALGO));
    ...
    private void processPaymentPayload(PaymentPayload payload, Booking booking) {
    ...
    private boolean verifyHmacSignatureWithSecret(String rawBody, String signature, String secret) {
        ...
        try {
            javax.crypto.Mac mac = javax.crypto.Mac.getInstance(HMAC_ALGO);
            mac.init(new javax.crypto.spec.SecretKeySpec(secret.getBytes(java.nio.charset.StandardCharsets.UTF_8), HMAC_ALGO));
